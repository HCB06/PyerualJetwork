from pyerualjetwork import ene_cpu, model_operations_cpu
import numpy as np
import cv2
...
genome_weights, genome_activations = ene_cpu.define_genomes(input_shape=7056, output_shape=2, population_size=100)
...
while True:
    ...
    for i in range(100):
        ...
        model_operations_cpu.save_model(model_name=f'mario{i}', model_path='', W=genome_weights[i], activation_potentiation=genome_activations[i], show_info=False)
...
model = model_operations_cpu.load_model(model_name='mario0', model_path='')

genome_weights = model[model_operations_cpu.get_weights()]
genome_activations = model[model_operations_cpu.get_act_pot()]
