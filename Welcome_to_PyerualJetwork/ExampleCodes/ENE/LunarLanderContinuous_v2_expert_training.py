...
for generation in range(max_generation):
    ...
    model_ops.save_model(model_name=f'lunar_expert_gen{generation}', model_type='MLP', W=genome_weights[np.argmax(rewards)], activation_potentiation=genome_activations[np.argmax(rewards)])
...
for i in range(100):
    ...
    while True:
        ...
        # Action Calculation
        action = ene.evaluate(Input=state, is_mlp=True, weights=genome_weights[np.argmax(rewards)], activation_potentiations=genome_activations[np.argmax(rewards)])
