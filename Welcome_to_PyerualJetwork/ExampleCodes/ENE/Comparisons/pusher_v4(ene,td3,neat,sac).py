...
from pyerualjetwork import ene_cpu, data_operations_cpu
import time
import torch
...
genome_weights, genome_activations = ene_cpu.define_genomes(input_shape, output_shape, population_size, neurons=[256, 256], activation_functions=['tanh', 'relu'])
...
for generation in range(max_generation):
    for individual in range(population_size):
        ...
        while True:
            # Action Calculation
            action = ene_cpu.evaluate(Input=state, weights=genome_weights[individual], is_mlp=True, activations=genome_activations[individual])
...
plt.plot(time_stamps_1, max_rewards_1, marker='o', linestyle='-', color='g', label="ENE (3 Hyperparameters Tuned)")
