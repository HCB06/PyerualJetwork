...
from pyerualjetwork import ene, data_operations
import time
import torch
...
genome_weights, genome_activations = ene.define_genomes(input_shape, output_shape, population_size, neurons=[256, 256], activation_functions=['tanh', 'relu'])
...
for generation in range(max_generation):
    for individual in range(population_size):
        ...
        while True:
            # Action Calculation
            action = ene.evaluate(Input=state, weights=genome_weights[individual], is_mlp=True, activations=genome_activations[individual])
