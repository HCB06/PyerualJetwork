...
x_train = data_ops.normalization(x_train)
x_test = data_ops.normalization(x_test)


activation_potentiation = ['bent_identity']

W = nn.fit(x_train, y_train, activations=activation_potentiation)

test_model = nn.evaluate(x_test, y_test, show_metrices=True, W=W, activations=activation_potentiation)
