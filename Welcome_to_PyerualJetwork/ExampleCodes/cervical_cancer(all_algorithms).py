...
y_train_decoded = data_operations_cpu.decode_one_hot(y_train)  # One-hot encoded y_train verilerini geri dönüştürme
...
genetic_optimizer = lambda *args, **kwargs: ene_cpu.evolver(*args, activation_mutate_add_prob=0, activation_selection_add_prob=0, policy='aggressive', **kwargs)

model = neu_cpu.learn(x_train, y_train, genetic_optimizer, gen=17, batch_size=0.05)
...
train_model = neu_cpu.evaluate(x_train, y_train, W=model[model_operations_cpu.get_weights()], activations=model[model_operations_cpu.get_act()])

print(Fore.GREEN + "\n------PLAN Modeli Sonuçları------" + Fore.RESET)
test_acc_plan = test_model[model_operations_cpu.get_acc()]
train_acc_plan = train_model[model_operations_cpu.get_acc()]
