...
# Karar sınırını görselleştir
plot_decision_boundary(x_test, y_test, model, feature_indices=[0, 1], model_name='Deep Learning', ax=ax, which_ax1=1, which_ax2=0)
...
print(classification_report(data_ops.decode_one_hot(y_test), data_ops.decode_one_hot(test_results[model_ops.get_preds_softmax()])))
# Karar sınırını görselleştir
plot_decision_boundary(x_test, y_test, model, feature_indices=[0, 1], model_name='PLAN', ax=ax, which_ax1=1, which_ax2=1)
plt.show()