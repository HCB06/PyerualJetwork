...
def plot_decision_boundary(x, y, model, feature_indices=[0, 1], h=0.02, model_name='str', ax=None, which_ax1=None, which_ax2=None, W=None, activation_potentiation=None):
    ...
    if model == 'PLAN':
        ...
        for i in range(len(grid_full)):
            Z[i] = np.argmax(model_operations_cpu.predict_from_memory(grid_full[i], W=W, activation_potentiation=activation_potentiation))
...
x_train, x_test, y_train, y_test = data_operations_cpu.split(X, y, test_size=0.4, random_state=42) # For less train data use this: (X, y, test_size=0.9, random_state=42)
...
model = neu_cpu.learn(x_train, y_train, genetic_optimizer, fit_start=True, gen=20)
...
print(classification_report(data_operations_cpu.decode_one_hot(y_test), data_operations_cpu.decode_one_hot(test_model[model_operations_cpu.get_preds_softmax()])))
# Karar sınırını görselleştir
