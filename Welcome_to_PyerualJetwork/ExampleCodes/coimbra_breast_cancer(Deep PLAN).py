...
genetic_optimizer = lambda *args, **kwargs: ene_cpu.evolver(*args, policy='aggressive', activation_selection_add_prob=0, activation_mutate_add_prob=0, **kwargs)

# Training Process
model = neu_cpu.learn(x_train, y_train, genetic_optimizer, fit_start=True, gen=60)
...
train_model = neu_cpu.evaluate(x_train, y_train, W=W, activations=activation_potentiation)
...
train_acc = train_model[model_operations_cpu.get_acc()]
...
model_operations_cpu.save_model(model_name='breast_cancer_coimbra',
...
for i in range(len(x_val)):
    Predict = model_operations_cpu.predict_from_storage(model_name='breast_cancer_coimbra', model_path='', Input=x_val[i])
