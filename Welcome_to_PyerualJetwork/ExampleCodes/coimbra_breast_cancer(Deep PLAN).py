...
# Training Process
model = neu.learn(x_train, y_train, genetic_optimizer, fit_start=True, gen=60)
...
train_model = neu.evaluate(x_train, y_train, W=W, activations=activation_potentiation)
...
for i in range(len(x_val)):
    Predict = model_operations.predict_from_storage(model_name='breast_cancer_coimbra', model_path='', Input=x_val[i])
