...
genetic_optimizer = lambda *args, **kwargs: ene.evolver(*args, strategy='normal_selective', show_info=True, **kwargs)
# neu Modeli
model = neu.learn(x_train, y_train, genetic_optimizer, fit_start=True, show_history=True, auto_normalization=False, batch_size=0.05,
                     gen=20)
...
train_model = neu.evaluate(x_train, y_train, W=W, activation_potentiation=activation_potentiation)
train_acc_neu = train_model[model_operations.get_acc()]
#print(f"PLAN Train Accuracy: {train_acc_plan:.4f}")

test_model = neu.evaluate(x_test, y_test, W=W, activations=activation_potentiation)
test_acc_neu = test_model[model_operations.get_acc()]
