...
y_train_decoded = data_operations_cpu.decode_one_hot(y_train)
lr_model.fit(x_train, y_train_decoded)

y_test_decoded = data_operations_cpu.decode_one_hot(y_test)
...
y_train_decoded_dl = data_operations_cpu.decode_one_hot(y_train)
...
y_pred_dl_classes = np.argmax(y_pred_dl, axis=1)  # Tahmin edilen sınıflar
y_test_decoded_dl = data_operations_cpu.decode_one_hot(y_test)
...
genetic_optimizer = lambda *args, **kwargs: ene_cpu.evolver(*args, strategy='normal_selective', show_info=True, **kwargs)
# neu Modeli
model = neu_cpu.learn(x_train, y_train, genetic_optimizer, fit_start=True, show_history=True, auto_normalization=False, batch_size=0.05,
                     gen=20)
...
train_model = neu_cpu.evaluate(x_train, y_train, W=W, activation_potentiation=activation_potentiation)
train_acc_neu = train_model[model_operations_cpu.get_acc()]
#print(f"PLAN Train Accuracy: {train_acc_plan:.4f}")

test_model = neu_cpu.evaluate(x_test, y_test, W=W, activations=activation_potentiation)
test_acc_neu = test_model[model_operations_cpu.get_acc()]
print(f"PLAN Test Accuracy: {test_acc_neu:.4f}")
print(classification_report(data_operations_cpu.decode_one_hot(y_test), data_operations_cpu.decode_one_hot(test_model[model_operations_cpu.get_preds()])))
