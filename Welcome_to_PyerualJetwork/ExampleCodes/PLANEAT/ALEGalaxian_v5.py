...
population_size = 50
genome_weights, genome_activations = planeat_cuda.define_genomes(input_shape, output_shape, population_size, hidden=2, neurons=[256, 256], activation_functions=['tanh', 'tanh'])
...
for generation in range(max_generation):
    ...
    for individual in range(population_size):
            ...
            while True:
                # Action Calculation
                action = planeat_cuda.evaluate(Input=cp.array(state, dtype=cp.float32), is_mlp=True, weights=genome_weights[individual], activation_potentiations=genome_activations[individual])
    ...
    genome_weights, genome_activations = planeat_cuda.evolver(
...
for i in range(100):
    while True:
        # Action Calculation
        action = planeat_cuda.evaluate(Input=cp.array(state, dtype=cp.float32), is_mlp=True, weights=genome_weights, activation_potentiations=genome_activations)
