...
def learn(x_train, y_train, iter, genetic_optimizer=None, gradient_optimizer=None, pop_size=None, fit_start=True, batch_size=1,
    ...
    * Gradient-based training is now supported via the backprop_train flag and gradient_optimizer.
    ...
    ```python
        ...
        model = nn.learn(x_train, y_train, gradient_optimizer=gradient_optimizer,
                         ...
                         fit_start=True, show_history=True, interval=16.67)
    ```
    :param pop_size: (int, optional): Population size of each generation for genetic optimization. Not required when backprop_train=True on a pure MLP. Default is None.
  ...
  # ── gradient train için ortak yardımcı ───────────────────────────────────
    def _run_backprop_train(init_weights, grad_epochs, final_activations, activation_potentiation):
        ...
        # step_size'ı N_total'a göre sınırla (batch boyutu veri sayısını geçemez)
        ...
        # Bilgi mesajı (opsiyonel - istenirse kaldırılabilir)
       ...
       # PLAN → PTNN dönüşümü
        if model_type == 'PLAN' and transfer_learning:
            ...
            if i == iter_copy[0]:
                ...
                # W[0]'ı genişlet: (output_size, input_size) → (neurons[0], input_size)
                plan_W = best_weight  # (2, input_size)
                extra_rows = neurons_copy[0] - len(y_train[0])  # 64 - 2 = 62
                ...
                expanded_W = np.vstack([plan_W, extra_W])  # (64, input_size)

                # neurons_copy'ye [2] ekleme — direkt [64, 64] kullan
                # activation_functions da ['linear'] ekleme — direkt ['relu', 'relu']
                weight_pop, act_pop = define_genomes(
                    input_shape=len(x_train[0]), output_shape=len(y_train[0]),
                    neurons=neurons_copy, activation_functions=activation_functions,
                    population_size=pop_size, dtype=dtype)

                # Tüm bireylere genişletilmiş W[0] yaz, sonraki katmanlar birim matris
        ...
        if model_type != 'PLAN' and transfer_learning is False and quick_start is False:
...
def grad(
    ...
    :param learning_rate: (float, optional): Step size for weight updates. Default: 0.001.
    ...
    #
    # List comprehension: döngü Python overhead'ını minimuma indirir,
